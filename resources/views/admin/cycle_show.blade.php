...
@section('content')
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">{{$cycle['attributes']['title']}}</li>
    </ol>
    <div class="row">

        <div class="col-9">
            <h2>#{{$cycle['attributes']['id']}} {{$cycle['attributes']['title']}}</h2>
            @if($errors->any())
            ...
            <div class="tab-content cycleTabContent" id="myTabContent">
                ...
                <div class="tab-pane fade cycleTabPane" id="profile" role="tabpanel" aria-labelledby="profile-tab">
                    <form id="cycleForm" method="POST" action="{{route('updateCycle')}}">
                        @csrf
                        {{ method_field('post') }}
                        <input type="hidden" name="created_by" value="{{$cycle['attributes']['created_by']}}">
                        <input type="hidden" name="id" value="{{$cycle['attributes']['id']}}">
                        <div class="form-group row">
                            <label for="title" class="col-sm-2 col-form-label">Title</label>
                            <div class="col-sm-10">
                                <input name="title" type="text" class="form-control" placeholder="title"
                                       value="{{$cycle['attributes']['title']}}" id="cycle_title">
                            </div>
                        </div>
                                ...
                                <div class="form-group row">
                                    <div class="col">
                                        <input type="text" class="form-control datepicker" placeholder="start date"
                                               name="start_date" value="{{$cycle['attributes']['start_date']}}"
                                               id="cycle_start_date">
                                    </div>
                                    <div class="col">
                                        <input type="text" class="form-control datepicker" placeholder="end date"
                                               name="end_date" value="{{$cycle['attributes']['end_date']}}" id="cycle_end_date">
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <input name="git_tag" type="text" class="form-control" placeholder="v0.0.0"
                                       value="{{$cycle['attributes']['git_tag']}}" id="git_tag">
                            </div>
                        </div>
